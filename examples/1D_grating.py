...
#  |       |              |       |       Duty Cycle:          50 %
#  |_______|              |_______|       Wavelength Interval: 0.9-1.1 microns
...
k0_l = np.linspace(0.9, 1.1, 201)  # Setting vector of wavelength

# Creating layers involved in structure
...
# Possible printing of G to see diffraction order
# for g in SiO.G:
...
for l in k0_l:
    for theta in theta_l:
        ...
        st.solve(k0, kx=np.sqrt(2.0) * k0 * np.sin(
            np.pi / 180.0 * theta))  # Solving system, kx is set as the parallel component of the wavevector (kx=k0*n*sin(theta))
        TM = st.get_T(st.NPW / 2, st.NPW / 2,
                      ordered='no')  # Extracting reflection and transmission for TM mode (pol along x)
        RM = st.get_R(st.NPW / 2, st.NPW / 2,
                      ordered='no')  # When dealing with uniform media the fundamental mode with pol along x is the NPW/2^th in unordered configuration
        TE = st.get_T(3 * st.NPW / 2, 3 * st.NPW / 2,
                      ordered='no')  # Extracting reflection and transmission for TE mode (pol along y)
        RE = st.get_R(3 * st.NPW / 2, 3 * st.NPW / 2,
                      ordered='no')  # When dealing with uniform media the fundamental mode with pol along y is the 3*NPW/2^th in unordered configuration
