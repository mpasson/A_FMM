...
if __name__ == '__main__':
    ...
    p0 = ax[0].contourf(np.squeeze(x), np.squeeze(y), np.sqrt(np.squeeze(eps)), levels=41)
    lay.mode(2.0)
    lay.mat_plot('test')
    data = lay.get_modal_field(3,x=vector, y=vector)
    to_plot = data.pivot(index = 'y', columns='x', values='Ex')
    p1 = ax[1].contourf(to_plot.columns, to_plot.index, np.abs(to_plot.values), levels=41)
    to_plot = data.pivot(index = 'y', columns='x', values='Ey')
    p2 = ax[2].contourf(to_plot.columns, to_plot.index, np.abs(to_plot.values), levels=41)
